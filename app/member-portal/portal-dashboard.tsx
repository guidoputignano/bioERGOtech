...
type Event = { id?: string; title: string; event_date: string; event_type: string; location: string; };
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 20, background: s.bg, color: s.color, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", letterSpacing: "0.01em" }}>{s.label}</span>;
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
        {pillars.map(p => (<button key={p} onClick={() => setFilter(p)} style={{ padding: "7px 16px", borderRadius: 24, fontFamily: "'DM Sans', sans-serif", border: `1.5px solid ${filter === p ? TEAL : BORDER}`, background: filter === p ? TEAL_LIGHT : CARD, color: filter === p ? TEAL_DARK : TEXT_MID, cursor: "pointer", fontSize: 12, fontWeight: 600, transition: "all 0.2s ease" }}>{p}</button>))}
      </div>
      ...
      {events.map((e, i) => {
        ...
        return (
          <div key={i} style={{ background: CARD, borderRadius: 16, padding: 24, border: `1.5px solid ${hl ? TEAL_MUTED : BORDER}`, boxShadow: SHADOW, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 20 }}><span style={{ color: TEXT_LIGHT }}><Icon name="mapPin" size={13} /></span><span style={{ fontSize: 13, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif" }}>{e.location}</span></div>
...
function AdminPanel() {
  const [tab, setTab] = useState<"applications" | "users" | "newsletter">("applications");
  ...
  useEffect(() => {
    setLoadingApps(true);
    fetch(`/api/admin/applications?status=${appFilter}`).then(r => r.json()).then(data => { setApplications(data.applications || []); setLoadingApps(false); }).catch(() => setLoadingApps(false));
  }, [appFilter]);

  useEffect(() => {
    if (tab !== "users") return;
    setLoadingUsers(true);
    fetch("/api/admin/users").then(r => r.json()).then(data => { setUsers(data.users || []); setLoadingUsers(false); }).catch(() => setLoadingUsers(false));
  }, [tab]);

  useEffect(() => {
    if (tab !== "newsletter") return;
    setLoadingNewsletter(true);
    fetch("/api/admin/newsletter").then(r => r.json()).then(data => { setSubscribers(data.subscribers || []); setLoadingNewsletter(false); }).catch(() => setLoadingNewsletter(false));
  }, [tab]);
  ...
  const exportCSV = () => {
    const csv = ["Name,Email,Source,Date"].concat(
      subscribers.map(s => `"${s.full_name || ""}","${s.email}","${s.source}","${new Date(s.subscribed_at).toLocaleDateString("en-GB")}"`)
    ).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `bioergotech-newsletter-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
            ...
            <div style={{ padding: "16px 24px", borderBottom: `1px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <button onClick={exportCSV}
                  style={{ padding: "7px 16px", borderRadius: 10, border: `1.5px solid ${TEAL}`, background: TEAL_LIGHT, color: TEAL_DARK, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
                  ↓ Export CSV
                </button>
              </div>
            </div>
