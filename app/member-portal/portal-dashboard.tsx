...
type Project = { id?: string; name: string; pillar: string; phase: string; status: string; lead: string; description: string; progress: number; color: string; };
...
function EventModal({ event, onClose, onSave }: { event: Partial<Event> | null; onClose: () => void; onSave: (e: Partial<Event>) => Promise<void>; }) {
  ...
  const inputStyle = { width: "100%", padding: "9px 14px", borderRadius: 10, border: `1.5px solid ${BORDER}`, fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: TEXT, outline: "none", background: "#FAFBFC" };
  ...
  const handle = async () => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : "Failed to save event."); }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", width: 520, background: CARD, borderRadius: 20, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", overflow: "hidden", animation: "fadeUp 0.2s ease both" }}>
        ...
        <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 16, maxHeight: "65vh", overflowY: "auto" }}>
          {error && <div style={{ padding: "10px 14px", borderRadius: 10, background: "#FDECF1", color: "#D63563", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>{error}</div>}
          {[{ label: "Title *", key: "title", placeholder: "e.g. Project Genesis Intro" }, { label: "Date *", key: "event_date", placeholder: "e.g. Feb 15 or Mar 2026" }, { label: "Location *", key: "location", placeholder: "e.g. Online or Taranto" }, { label: "Video Link", key: "video_link", placeholder: "https://..." }].map(f => (
            <div key={f.key}>
              <div style={{ fontSize: 12, fontWeight: 600, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif", marginBottom: 6 }}>{f.label}</div>
              <input value={(form as Record<string, string>)[f.key] || ""} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder} style={inputStyle} />
            </div>
          ))}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif", marginBottom: 6 }}>Event Type *</div>
            <select value={form.event_type} onChange={e => setForm(p => ({ ...p, event_type: e.target.value }))} style={inputStyle}>
              {EVENT_TYPES.map(t => (<option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>))}
            </select>
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif", marginBottom: 6 }}>Description</div>
            <textarea value={form.description || ""} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} placeholder="Optional event description..." style={{ ...inputStyle, resize: "vertical" as const }} />
          </div>
          <div style={{ display: "flex", gap: 16 }}>
            {[{ key: "is_public", label: "Public" }, { key: "is_approved", label: "Approved" }].map(f => (
              <label key={f.key} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif" }}>
                <input type="checkbox" checked={(form as Record<string, boolean>)[f.key] ?? true} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.checked }))} style={{ accentColor: TEAL, width: 15, height: 15 }} />
                {f.label}
              </label>
            ))}
          </div>
        </div>
        <div style={{ padding: "18px 28px", borderTop: `1px solid ${BORDER}`, display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button onClick={handle} disabled={saving} style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_DARK})`, color: "#fff", fontSize: 13, fontWeight: 700, cursor: saving ? "default" : "pointer", fontFamily: "'DM Sans', sans-serif", opacity: saving ? 0.7 : 1, boxShadow: `0 2px 8px ${TEAL}33` }}>
          ...
          {events.slice(0, 5).map((e, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "11px 0", borderBottom: i < 4 ? `1px solid ${BORDER}` : "none" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", marginTop: 2, display: "flex", alignItems: "center", gap: 4 }}><Icon name="mapPin" size={11} /> {e.location}</div>
              </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 14, alignItems: "center" }}>
        ...
        <button style={{ padding: "11px 20px", borderRadius: 12, border: "none", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_DARK})`, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {isAdmin && (
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => onEdit?.(e)} style={{ background: "#F3F5F8", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: TEXT_MID }} title="Edit"><Icon name="edit" size={13} /></button>
                    <button onClick={() => { if (confirm("Delete this event?")) onDelete?.(e.id!); }} style={{ background: "#FDECF1", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: "#D63563" }} title="Delete"><Icon name="trash" size={13} /></button>
                  </div>
                )}
              </div>
            </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {(!members || members.length === 0) ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
          {[1,2,3,4,5,6].map(i => (<div key={i} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 20, boxShadow: SHADOW, height: 80, opacity: 0.4 }} />))}
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
          {members.map((m, i) => (
      ...
      <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 520, background: CARD, boxShadow: "-4px 0 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column", overflow: "hidden", animation: "slideIn 0.25s ease both" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {app.areas_of_interest && app.areas_of_interest.length > 0 && (
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 7 }}>
                {app.areas_of_interest.map(a => (<span key={a} style={{ fontSize: 12, padding: "4px 12px", borderRadius: 20, background: TEAL_LIGHT, color: TEAL_DARK, fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>{PILLAR_LABELS[a] ?? a}</span>))}
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Add notes visible only to admins..." rows={3} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${BORDER}`, fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: TEXT, outline: "none", resize: "vertical" as const, background: "#FAFBFC" }} />
          </div>
          {app.application_status === "pending" && (
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {(["member", "partner"] as PartnershipLevel[]).map(l => { const lbl = PARTNERSHIP_LABELS[l]; return (<button key={l} onClick={() => setLevel(l)} style={{ padding: "7px 18px", borderRadius: 10, border: `1.5px solid ${level === l ? lbl.color : BORDER}`, background: level === l ? lbl.bg : CARD, color: level === l ? lbl.color : TEXT_LIGHT, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>{lbl.label}</button>); })}
              </div>
            </div>
          )}
        </div>
        {app.application_status === "pending" && (
          <div style={{ padding: "18px 28px", borderTop: `1px solid ${BORDER}`, display: "flex", gap: 10 }}>
            ...
            <button onClick={() => handle("approve")} disabled={acting} style={{ flex: 2, padding: "11px 0", borderRadius: 12, border: "none", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_DARK})`, color: "#fff", fontSize: 13, fontWeight: 700, cursor: acting ? "default" : "pointer", fontFamily: "'DM Sans', sans-serif", boxShadow: `0 2px 8px ${TEAL}33`, opacity: acting ? 0.6 : 1 }}>{acting ? "Processing…" : `Approve as ${PARTNERSHIP_LABELS[level].label}`}</button>
          </div>
        )}
        {app.application_status !== "pending" && (
...
// ─── ADMIN PANEL ─────────────────────────────────────────────────────────────
function AdminPanel({ onEventsChanged }: { onEventsChanged?: () => void }) {
  const [tab, setTab] = useState<"applications" | "users" | "events" | "newsletter">("applications");
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/newsletter").then(r => r.json()).then(d => { setSubscribers(d.subscribers || []); setLoadingNewsletter(false); }).catch(() => setLoadingNewsletter(false));
  }, [tab]);

  useEffect(() => {
    if (tab !== "events") return;
    setLoadingEvents(true);
    fetch("/api/admin/events").then(r => r.json()).then(d => { setAdminEvents(d.events || []); setLoadingEvents(false); }).catch(() => setLoadingEvents(false));
  }, [tab]);
  ...
  const handleAction = async (id: string, action: "approve" | "decline", notes: string, level: PartnershipLevel) => {
    ...
    try {
      ...
      else { setActionMessage({ type: "error", text: data.error || "Failed to process application." }); }
    } catch { setActionMessage({ type: "error", text: "Network error. Please try again." }); }
  };
  ...
  const handleSaveEvent = async (event: Partial<Event>) => {
    ...
    if (isEdit) { setAdminEvents(prev => prev.map(e => e.id === event.id ? data.event : e)); }
    else { setAdminEvents(prev => [data.event, ...prev]); }
    onEventsChanged?.();
  };

  const handleDeleteEvent = async (id: string) => {
    const res = await fetch("/api/admin/events", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    if (res.ok) { setAdminEvents(prev => prev.filter(e => e.id !== id)); onEventsChanged?.(); }
  };
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `bioergotech-newsletter-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
  };
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ background: "#FDECF1", border: "1px solid #F9C3CE", borderRadius: 14, padding: "16px 22px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div><div style={{ fontSize: 14, fontWeight: 700, color: TEXT, fontFamily: "'Sora', sans-serif" }}>Admin Panel</div><div style={{ fontSize: 12, color: TEXT_MID, fontFamily: "'DM Sans', sans-serif" }}>Manage applications, users, events and newsletter subscribers.</div></div>
        </div>
        ...
        <div style={{ display: "flex", gap: 4, background: "#F3F5F8", borderRadius: 12, padding: 4, width: "fit-content" }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 18px", borderRadius: 9, border: "none", background: tab === t.id ? CARD : "transparent", color: tab === t.id ? TEXT : TEXT_LIGHT, fontSize: 13, fontWeight: tab === t.id ? 700 : 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", boxShadow: tab === t.id ? SHADOW : "none", transition: "all 0.15s ease" }}>
              ...
              {t.badge != null && (<span style={{ background: "#E74C6F", color: "#fff", borderRadius: 20, fontSize: 10, fontWeight: 700, padding: "1px 7px", fontFamily: "'DM Sans', sans-serif" }}>{t.badge}</span>)}
            </button>
          ))}
        </div>
          ...
          <>
            <div style={{ display: "flex", gap: 8 }}>
              {(["pending", "approved", "declined", "all"] as const).map(s => (
                <button key={s} onClick={() => setAppFilter(s)} style={{ padding: "6px 16px", borderRadius: 20, border: `1.5px solid ${appFilter === s ? (s === "pending" ? "#F0A500" : s === "approved" ? TEAL : s === "declined" ? "#E74C6F" : BORDER) : BORDER}`, background: appFilter === s ? (s === "pending" ? "#FFF8E6" : s === "approved" ? TEAL_LIGHT : s === "declined" ? "#FDECF1" : "#F3F5F8") : CARD, color: appFilter === s ? (s === "pending" ? "#C48700" : s === "approved" ? TEAL_DARK : s === "declined" ? "#D63563" : TEXT_MID) : TEXT_LIGHT, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", textTransform: "capitalize" as const }}>{s}</button>
              ))}
            </div>
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden", boxShadow: SHADOW }}>
              <div style={{ padding: "16px 24px", borderBottom: `1px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: TEXT, fontFamily: "'Sora', sans-serif" }}>{appFilter === "all" ? "All Applications" : `${appFilter.charAt(0).toUpperCase() + appFilter.slice(1)} Applications`}</h3>
                <span style={{ fontSize: 12, color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif" }}>{applications.length} total</span>
              </div>
              {loadingApps ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Loading applications…</div>)
                : applications.length === 0 ? (
                  <div style={{ padding: 48, textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 13, color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", marginTop: 4 }}>{appFilter === "pending" ? "New applications will appear here once members apply." : "Nothing to show here."}</div>
                  </div>
                ) : (
                  <div>
                    {applications.map((app, i) => {
                      ...
                      return (
                        <div key={app.id} onClick={() => setSelectedApp(app)} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 140px 120px 80px", padding: "16px 24px", borderBottom: i < applications.length - 1 ? `1px solid ${BORDER}` : "none", alignItems: "center", cursor: "pointer", transition: "background 0.15s ease", fontFamily: "'DM Sans', sans-serif" }}
        ...
        {tab === "users" && (
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden", boxShadow: SHADOW }}>
            ...
            {loadingUsers ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Loading users…</div>)
              : users.length === 0 ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>No users found.</div>)
              : (
                <div>
        ...
        {tab === "events" && (
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden", boxShadow: SHADOW }}>
            <div style={{ padding: "16px 24px", borderBottom: `1px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setEventModal({ open: true, event: null })} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${TEAL}, ${TEAL_DARK})`, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", boxShadow: `0 2px 8px ${TEAL}33` }}>
                <Icon name="plus" size={14} /> Add Event
              </button>
            </div>
            {loadingEvents ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Loading events…</div>)
              : adminEvents.length === 0 ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>No events yet. Add one above.</div>)
              : (
                <div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 120px 100px 90px", padding: "12px 24px", background: "#FAFBFC", borderBottom: `1px solid ${BORDER}`, fontSize: 11, color: TEXT_LIGHT, textTransform: "uppercase" as const, letterSpacing: "0.07em", fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>
                    <span>Title</span><span>Date</span><span>Location</span><span>Type</span><span>Actions</span>
                  </div>
                  {adminEvents.map((e, i) => (
                    <div key={e.id} style={{ display: "grid", gridTemplateColumns: "1fr 100px 120px 100px 90px", padding: "14px 24px", borderBottom: i < adminEvents.length - 1 ? `1px solid ${BORDER}` : "none", alignItems: "center", fontFamily: "'DM Sans', sans-serif" }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>{e.title}</div>
                        {!e.is_approved && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: "#FFF8E6", color: "#C48700", fontWeight: 700 }}>Pending</span>}
                      </div>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => setEventModal({ open: true, event: e })} style={{ background: "#F3F5F8", border: "none", borderRadius: 7, padding: "6px 8px", cursor: "pointer", color: TEXT_MID }} title="Edit"><Icon name="edit" size={14} /></button>
                        <button onClick={() => { if (confirm("Delete this event?")) handleDeleteEvent(e.id!); }} style={{ background: "#FDECF1", border: "none", borderRadius: 7, padding: "6px 8px", cursor: "pointer", color: "#D63563" }} title="Delete"><Icon name="trash" size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tab === "newsletter" && (
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden", boxShadow: SHADOW }}>
            ...
            {loadingNewsletter ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Loading subscribers…</div>)
              : subscribers.length === 0 ? (<div style={{ padding: 40, textAlign: "center", color: TEXT_LIGHT, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>No subscribers yet.</div>)
              : (
                <div>
...
export default function BioERGOtechPortal({ user }: { user: PortalUser }) {
  ...
  const visibleNav = navItems.filter(item => { if (item.id === "admin") return partnershipLevel === "admin"; return true; });
    ...
    switch (activeNav) {
      ...
      case "events": return (
        <SectionWrapper sectionId="events" sectionName="Events" partnershipLevel={partnershipLevel}>
          <EventsView events={events} isAdmin={partnershipLevel === "admin"} onEdit={e => { }} onDelete={async id => { await fetch("/api/admin/events", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }); fetchEvents(); }} />
        </SectionWrapper>
      );
      ...
      case "admin": return partnershipLevel === "admin" ? <AdminPanel onEventsChanged={fetchEvents} /> : null;
