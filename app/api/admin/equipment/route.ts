  ...
  try {
    ...
    if (!payload.name || !payload.location) {
      return NextResponse.json(
        { error: "Equipment name and location are required" },
        { status: 400 }
      );
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    if (fields.proposed_by_email !== undefined) {
      updatePayload.proposed_by_email = fields.proposed_by_email?.trim() || null;
    }
    if (fields.proposed_by_name !== undefined) {
      updatePayload.proposed_by_name = fields.proposed_by_name?.trim() || null;
    }
    if (fields.contact_email !== undefined) {
      updatePayload.contact_email = fields.contact_email?.trim() || null;
    }
    if (fields.contact_phone !== undefined) {
      updatePayload.contact_phone = fields.contact_phone?.trim() || null;
    }
    if (fields.image_url !== undefined) {
      updatePayload.image_url = fields.image_url?.trim() || null;
    }
    ...
    if (fields.admin_notes !== undefined) {
      updatePayload.admin_notes = fields.admin_notes?.trim() || null;
    }
    if (fields.is_available !== undefined) {
      updatePayload.is_available = fields.is_available;
    }
    if (fields.utilization !== undefined) {
      updatePayload.utilization = Number(fields.utilization) || 0;
    }
    if (fields.cost_savings_text !== undefined) {
      updatePayload.cost_savings_text = fields.cost_savings_text?.trim() || null;
    }
    if (fields.bookings_count !== undefined) {
      updatePayload.bookings_count = Number(fields.bookings_count) || 0;
    }
