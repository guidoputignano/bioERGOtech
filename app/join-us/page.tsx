...
const SCIENTIFIC_PILLARS = [
  { value: "scientific_innovation", label: "Scientific Research and Innovation", desc: "Research projects, scientific exploration, and new ideas." },
  { value: "education_dissemination", label: "Training and Scientific Dissemination", desc: "Education, public engagement, and knowledge-sharing activities." },
  { value: "technology_transfer", label: "Technology Development and Transfer", desc: "Turning ideas into practical tools, products, and startup support." },
  { value: "biotech_collaboration", label: "Biotech Collaboration and Research Networks", desc: "Cross-organisation and interdisciplinary biotech collaboration." },
  { value: "sustainability_social_impact", label: "Sustainability and Social Responsibility", desc: "Responsible innovation and long-term societal value." },
  { value: "clinical_validation", label: "Clinical Trials and Therapeutic Validation", desc: "Clinical relevance, validation, and therapeutic application." },
  { value: "support_services", label: "Support Services", desc: "Operational, technical, or enabling support for the ecosystem." },
  { value: "other", label: "Other", desc: "Any other area not captured above." },
];
        ...
        return (
          <div key={s.num} className="flex items-center">
            <div className="flex flex-col items-center gap-1.5">
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300" style={{ background: done ? "var(--primary)" : active ? "var(--primary)" : "#E8EDF3", color: done || active ? "#fff" : "#8896A6", boxShadow: active ? "0 0 0 4px #2EC4B620" : "none" }}>
                {done ? (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>) : (s.num + 1)}
              </div>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {PARTNERSHIP_TYPES.map((type) => {
          ...
          return (
            <button key={type.value} type="button" onClick={() => onChange("partnership_type", type.value)} className="text-left rounded-2xl border p-5 transition-all duration-200" style={{ borderColor: selected ? "var(--primary)" : "#E8EDF3", background: selected ? "#E8F8F6" : "#fff", boxShadow: selected ? "0 0 0 3px #2EC4B620" : "none" }}>
...
function Step1({ data, onChange }: { data: FormData; onChange: (k: keyof FormData, v: string) => void; }) {
  return (
    <div className="flex flex-col gap-5">
      ...
      <Field label="Referral Code" hint="If a bioERGOtech member referred you, enter their referral code here (optional).">
        <input className={inputClass} placeholder="e.g. AB12CD34" value={data.referred_by} onChange={(e) => onChange("referred_by", e.target.value.toUpperCase())} maxLength={8} />
      </Field>
...
function OrgSearch({ data, onSelectOrg, onChange }: { data: FormData; onSelectOrg: (org: OrgResult | null) => void; onChange: (k: keyof FormData, v: string) => void; }) {
      ...
      <Field label="Organisation Name" required>
        <div className="relative" ref={dropdownRef}>
          <div className="relative">
            <input className={inputClass} placeholder="Search by organisation name..." value={query} onChange={(e) => { setQuery(e.target.value); onChange("organisation_name", e.target.value); onChange("organisation_id", ""); setIsNewOrg(true); }} onFocus={() => { if (results.length > 0) setShowDropdown(true); }} />
            {searching && (<div className="absolute right-3 top-1/2 -translate-y-1/2"><svg className="animate-spin w-4 h-4 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg></div>)}
            {data.organisation_id && (<button onClick={() => { setQuery(""); setIsNewOrg(true); onSelectOrg(null); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" type="button"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>)}
          </div>
          {data.organisation_id && (<div className="mt-2 flex items-center gap-2 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#2EC4B6", background: "#E8F8F6" }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1A9E92" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg><span className="font-semibold" style={{ color: "#1A9E92" }}>Linked to existing organisation</span></div>)}
          {showDropdown && results.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-gray-100 shadow-lg z-50 overflow-hidden">
              {results.map((org) => (
                <button key={org.id} onClick={() => { setQuery(org.name); setShowDropdown(false); setIsNewOrg(false); onSelectOrg(org); }} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0" type="button">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-xs font-bold" style={{ background: typeBgs[org.org_type] || "#F3F5F8", color: typeColors[org.org_type] || "#8896A6" }}>{org.name.charAt(0)}</div>
                  <div className="flex-1 min-w-0"><div className="text-sm font-semibold text-gray-800 truncate">{org.name}</div><div className="text-xs text-gray-500">{[org.city, org.country].filter(Boolean).join(", ")}</div></div>
                  <span className="text-xs px-2 py-1 rounded-lg flex-shrink-0" style={{ background: typeBgs[org.org_type] || "#F3F5F8", color: typeColors[org.org_type] || "#8896A6" }}>{typeLabels[org.org_type] || org.org_type}</span>
                </button>
              ))}
      ...
      {(isNewOrg || data.organisation_id) && query.trim().length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Country" required><input className={inputClass} placeholder="e.g. Italy" value={data.country} onChange={(e) => onChange("country", e.target.value)} disabled={!!data.organisation_id} /></Field>
            <Field label="City" required><input className={inputClass} placeholder="e.g. Taranto" value={data.city} onChange={(e) => onChange("city", e.target.value)} disabled={!!data.organisation_id} /></Field>
          </div>
...
function Step3({ data, onTogglePillar, onChange }: { data: FormData; onTogglePillar: (v: string) => void; onChange: (k: keyof FormData, v: string) => void; }) {
  return (
    <div className="flex flex-col gap-5">
      ...
      <Field label="Areas of Interest" required hint="Select one or more areas relevant to your interest in the Foundation.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
          {SCIENTIFIC_PILLARS.map((p) => {
            ...
            return (
              <button key={p.value} type="button" onClick={() => onTogglePillar(p.value)} className="flex items-start gap-3 px-4 py-3 rounded-xl border text-left transition-all duration-200" style={{ borderColor: selected ? "var(--primary)" : "#E8EDF3", background: selected ? "#E8F8F6" : "#fff", color: selected ? "#1A9E92" : "#4A5568" }}>
                <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0 mt-0.5 transition-all" style={{ background: selected ? "var(--primary)" : "#E8EDF3" }}>
                ...
                <div><div className="text-sm font-medium">{p.label}</div><div className="text-xs text-gray-500 mt-1">{p.desc}</div></div>
              </button>
            );
          })}
        </div>
      ...
      {data.partnership_type === "strategic_partner" && (
        <>
          <Field label="Type of Strategic Contribution" hint="Optional.">
          ...
          <Field label="Expected Timeline" hint="Optional.">
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-2xl border border-gray-100 bg-gray-50 px-5 py-2">
        ...
        {data.referred_by && <Row label="Referral Code" value={data.referred_by} />}
        {data.partnership_type === "strategic_partner" && (<><Row label="Mobile Phone" value={data.mobile_phone} /><Row label="Additional Email" value={data.strategic_email} /><Row label="Codice Fiscale / P.IVA" value={data.tax_id} /></>)}
      </div>
      <div className="rounded-2xl border border-gray-100 bg-gray-50 px-5 py-2">
      ...
      <div className="rounded-2xl border border-gray-100 bg-gray-50 px-5 py-2">
        ...
        {data.partnership_type === "strategic_partner" && (<><Row label="Strategic Contribution" value={data.collaboration_type} /><Row label="Timeline" value={data.expected_timeline} /></>)}
      </div>
      <div className="flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all" style={{ borderColor: data.newsletter_consent ? "#2EC4B6" : "#E8EDF3", background: data.newsletter_consent ? "#E8F8F6" : "#FAFBFC" }} onClick={() => onToggleNewsletter(!data.newsletter_consent)}>
        <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0 mt-0.5 transition-all" style={{ background: data.newsletter_consent ? "var(--primary)" : "#E8EDF3", border: `2px solid ${data.newsletter_consent ? "var(--primary)" : "#D1D5DB"}` }}>
...
export default function JoinUs() {
  ...
  const toggleNewsletter = (value: boolean) => setForm((prev) => ({ ...prev, newsletter_consent: value }));
  const togglePillar = (value: string) => setForm((prev) => ({ ...prev, areas_of_interest: prev.areas_of_interest.includes(value) ? prev.areas_of_interest.filter((v) => v !== value) : [...prev.areas_of_interest, value] }));

  const ORG_TYPE_MAP: Record<string, string> = { Foundation: "other", University: "university", Startup: "startup", SME: "sme", "Clinical Center": "hospital", Investor: "investor", "International Partner": "international_partner" };

  const handleSelectOrg = (org: OrgResult | null) => {
    if (org) {
      setForm((prev) => ({ ...prev, organisation_id: org.id, organisation_name: org.name, organisation_type: ORG_TYPE_MAP[org.org_type] || org.org_type.toLowerCase(), city: org.city || "", country: org.country || "" }));
    } else {
      setForm((prev) => ({ ...prev, organisation_id: "", organisation_name: "", organisation_type: "", city: "", country: "", organisation_website: "" }));
    }
  };
            ...
            <div className="max-w-2xl mx-auto">
              <div className="card" style={{ borderRadius: 24, padding: "40px" }}>
                ...
                <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-100">
                  <button onClick={() => { if (step > 0) setStep((s) => (s - 1) as Step); }} disabled={step === 0} className="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all" style={{ background: step === 0 ? "#F3F5F8" : "#fff", border: "1.5px solid #E8EDF3", color: step === 0 ? "#C5CED8" : "#4A5568", cursor: step === 0 ? "default" : "pointer" }}>← Back</button>
                  <span className="text-xs text-gray-400 font-medium">Step {step + 1} of 5</span>
                  {step < 4 ? (
                    <button onClick={() => { if (step < 4) setStep((s) => (s + 1) as Step); }} disabled={!canAdvance()} className="px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all" style={{ background: canAdvance() ? "linear-gradient(135deg, #2EC4B6, #1A9E92)" : "#E8EDF3", color: canAdvance() ? "#fff" : "#C5CED8", cursor: canAdvance() ? "pointer" : "default", boxShadow: canAdvance() ? "0 2px 8px #2EC4B633" : "none" }}>Continue →</button>
                  ) : (
                    <button onClick={handleSubmit} disabled={submitting} className="px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all flex items-center gap-2" style={{ background: submitting ? "#E8EDF3" : "linear-gradient(135deg, #2EC4B6, #1A9E92)", color: submitting ? "#C5CED8" : "#fff", cursor: submitting ? "default" : "pointer", boxShadow: submitting ? "none" : "0 2px 8px #2EC4B633" }}>{submitting ? "Submitting..." : "Submit Application ✓"}</button>
                  )}
                </div>
              </div>
