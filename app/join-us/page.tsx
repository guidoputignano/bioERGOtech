...
interface FormData {
  ...
  // Step 4 — Review & submit (no extra fields)
}
          ...
          <div key={s.num} className="flex items-center">
            <div className="flex flex-col items-center gap-1.5">
              ...
              <span
                ...
                style={{ color: active ? "var(--primary)" : "#8896A6" }}
              >
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div
                ...
                style={{ background: done ? "var(--primary)" : "#E8EDF3" }}
              />
            )}
          </div>
        );
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-gray-500">Tell us about the organisation joining the ecosystem.</p>
      </div>
      <Field label="Organisation Name" required>
        <input
          className={inputClass}
          placeholder="e.g. BioTech Solutions S.r.l."
      ...
      <Field label="Organisation Type" required>
        <select
          ...
          onChange={(e) => onChange("organisation_type", e.target.value)}
        >
      ...
      <Field label="Website" hint="Optional — helps us learn more about your organisation.">
        <input
          ...
          onChange={(e) => onChange("organisation_website", e.target.value)}
        />
      </Field>
    </div>
  );
    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Help us understand how you fit into the ecosystem.
        </p>
      </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: selected ? "var(--primary)" : "#E8EDF3",
                  }}
                >
                  {selected && (
      ...
      <Field
        label="What do you bring to the ecosystem?"
        required
        hint="Skills, resources, networks, equipment, expertise — be specific."
      >
        <textarea
          className={inputClass}
          rows={4}
          placeholder="e.g. We bring a clinical network of 12 hospitals in Puglia with access to patient cohorts for oncology research, plus a team of 3 bioinformaticians..."
          value={data.what_you_bring}
          onChange={(e) => onChange("what_you_bring", e.target.value)}
        />
      </Field>
      ...
      <Field
        ...
        required
        hint="Collaborators, resources, funding, expertise — what does success look like for you?"
      >
        <textarea
          className={inputClass}
          rows={4}
          placeholder="e.g. We are looking for computational biology partners to help analyse our multi-omics datasets and co-develop AI diagnostic tools for early cancer detection..."
          value={data.what_you_seek}
          onChange={(e) => onChange("what_you_seek", e.target.value)}
        />
      </Field>
    </div>
  );
...
function Step4({ data }: { data: FormData }) {
  ...
  const pillars = SCIENTIFIC_PILLARS.filter((p) =>
    data.areas_of_interest.includes(p.value)
  ).map((p) => p.label);

  const Row = ({ label, value }: { label: string; value: string }) => (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1 py-3 border-b border-gray-100 last:border-0">
      <span className="text-xs font-bold text-gray-400 uppercase tracking-wide w-40 flex-shrink-0 pt-0.5">
        {label}
      </span>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-2xl border border-gray-100 bg-gray-50 px-5 py-2">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider pt-3 pb-1">
          Contact
        </p>
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-teal-50 border border-teal-100">
        <svg className="flex-shrink-0 mt-0.5" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A9E92" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="10" />
          <line x1="12" y1="8" x2="12" y2="12" />
          <line x1="12" y1="16" x2="12.01" y2="16" />
        </svg>
        <p className="text-sm text-teal-800">
          Your application will be reviewed by the bioERGOtech team. You will receive a
          confirmation email and we will be in touch within <strong>5 business days</strong>.
        </p>
      </div>
    </div>
  );
...
export default function JoinUs() {
  ...
  const canAdvance = (): boolean => {
    if (step === 1)
      return !!(form.full_name.trim() && form.contact_role.trim() && form.email.trim());
    if (step === 2)
      return !!(
    ...
    if (step === 3)
      return !!(
        form.areas_of_interest.length > 0 &&
        form.what_you_bring.trim() &&
        form.what_you_seek.trim()
      );
    return true;
  };
  ...
  try {
    ...
    setError(
      err instanceof Error
        ? err.message
        : "Something went wrong. Please try again or email join@bioergotech.org."
    );
  } finally {
    setSubmitting(false);
  }
          ...
          <section className="section bg-light-gray min-h-screen flex items-center justify-center">
            <div className="max-w-md mx-auto px-6 text-center">
              ...
              <p className="text-gray-600 mb-2">
                Thank you, <strong>{form.full_name}</strong>. Your application for{" "}
                <strong>{form.organisation_name}</strong> has been received.
              </p>
        ...
        <section className="bg-light-gray" style={{ padding: "60px 0 40px" }}>
          <div className="container mx-auto px-6 text-center">
            ...
            <p className="text-lg max-w-2xl mx-auto text-gray-600">
              Apply for membership to bioERGOtech Foundation. We review every application
              personally and aim to respond within 5 business days.
            </p>
          </div>
        </section>

        {/* Find Your Path — kept from original */}
        <section className="section" id="your-path">
          <div className="container mx-auto px-6">
            <h2 className="section-title text-center block">Find Your Path</h2>
            <p className="text-lg text-gray-700 max-w-3xl mx-auto text-center mb-12">
              We work with diverse communities across the life sciences ecosystem.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                ...
                <div key={p.title} className="card">
                  <div className="mb-4">
                    <i className={`fas ${p.icon} text-4xl`} style={{ color: "var(--primary)" }} />
                  </div>
          ...
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div className="flex justify-center">
                <img
                  src="/assets/images/Join-us/Community.webp"
                  alt="Our Community"
          ...
          <div className="container mx-auto px-6">
            <div className="text-center mb-10">
              ...
              <p className="text-gray-600 max-w-xl mx-auto">
                Complete the form below. All applications are reviewed by the Foundation team.
              </p>
            </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="card" style={{ borderRadius: 24, padding: "40px" }}>
                ...
                <div style={{ minHeight: 340 }}>
                  ...
                  {step === 3 && (
                    <Step3 data={form} onTogglePillar={togglePillar} onChange={update} />
                  )}
                ...
                <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-100">
                  <button
                    ...
                    style={{
                      background: step === 1 ? "#F3F5F8" : "#fff",
                      border: "1.5px solid #E8EDF3",
                      color: step === 1 ? "#C5CED8" : "#4A5568",
                    ...
                    <button
                      ...
                      style={{
                        background: canAdvance()
                          ? "linear-gradient(135deg, #2EC4B6, #1A9E92)"
                          : "#E8EDF3",
